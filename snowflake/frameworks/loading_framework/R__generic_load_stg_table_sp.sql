CREATE OR REPLACE PROCEDURE COM.GENERIC_LOAD_STG_TABLE_SP("EXECUTION_ID" VARCHAR(16777216), "START_TIME" VARCHAR(16777216), "CONSTRAINTS_LIST" VARCHAR(16777216))
...
try {
   ...
   snowflake.execute(
    ...
    var file_prefix_name_case_param = `{"json_key":"file_prefix_name_case","default_value":"lower"}`;
	var file_prefix_name_case = call_common_function('get_value_with_default' , file_prefix_name_case_param, CONSTRAINTS_LIST);

    if (file_prefix_name_case == 'as_is'){
        var file_prefix_path = constraints_list_json["file_prefix_path"];
    }
    else if (file_prefix_name_case == 'upper'){
        var file_prefix_path = constraints_list_json["file_prefix_path"].toUpperCase();
    }
    else{
        var file_prefix_path = constraints_list_json["file_prefix_path"].toLowerCase();
    }
    ...
    if (load_type == 'all_with_add_on') {
        ...
        //---check for audit columns as well, if audit columns are present then we add them with custom cols
        var common_fun_param = `{"execution_id":"` + EXECUTION_ID + 
                               `"}`;
	    var audit_query_columns = call_common_function('fetch_audit_columns' , common_fun_param, CONSTRAINTS_LIST);
        query_columns = query_columns +','+ audit_query_columns;
